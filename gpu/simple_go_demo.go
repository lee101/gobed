...
func main() {
	...
	fmt.Println("\n🏗️ Test 2: Create GPU Search Engine")
	...
	fmt.Println("\n⚙️ Test 3: Regular Engine with GPU Config")
	...
	for i, doc := range docs {
		...
		if err != nil {
			...
			fmt.Printf("   ⚠️  Indexing failed (expected without GPU): %v\n", err)
			break
		}
	}
	...
	if err != nil {
		fmt.Printf("   ⚠️  Search failed (expected without GPU): %v\n", err)
	} else {
	...
	fmt.Println("   🏗️ GPU search engine constructors working")
