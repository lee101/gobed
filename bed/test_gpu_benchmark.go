...
func main() {
	...
	for _, query := range queries {
		// Test CPU version
		startCPU := time.Now()
		cpuCmd := exec.Command("./bed_ultra_fixed", "-dir", "testdata", "-k", "5", query)
		cpuOutput, cpuErr := cpuCmd.Output()
		cpuTime := time.Since(startCPU)

		// Test GPU version (if working)
		startGPU := time.Now()
		gpuCmd := exec.Command("./bed_gpu_3090", "-dir", "testdata", "-k", "5", query)
	...
	batchCmd := exec.Command("./bed_gpu_3090", "-batch", "-dir", "testdata", "-k", "10", "test")
