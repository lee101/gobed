...
func (idx *Indexer) Index(options IndexOptions) error {
	...
	// Progress monitoring has a dedicated stop signal so it can terminate cleanly
	// once channels drain.
	...
	go func() {
		...
		idx.monitorProgress(options, stopProgress)
	}()
...
func (idx *Indexer) monitorProgress(options IndexOptions, stop <-chan struct{}) {
